...
public class GoodSingleActivity extends Activity implements OnClickListener {
	...
	public class getGoodsXqAsnyTask extends
		...
		protected Boolean doInBackground(String[] arg0) {
			try {
				...
				String msg = "*AIS|RQ|PD01|" + cStoreNo + "|" + cGoodsNo
						+ "|AIE#";
				...
				if (temp == null) {
					...
					if ("*AIS".equalsIgnoreCase(temp[0])
							&& "RS".equals(temp[1]) && "PD01".equals(temp[2])
							&& temp[3].equals("01")) {
						Gson gson = new Gson();
	...
	public void lijigoumai(View v) {
		String gdnum = gd_num_edit.getText().toString();
		if (TextUtils.isEmpty(gdnum)) {
			PromptManager.showMyToast(getApplicationContext(), "商品数量必须是1~99整数");
			return;
		}
		int nums;
		try {
			nums = Integer.parseInt(gdnum);
